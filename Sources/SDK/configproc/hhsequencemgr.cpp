...
bool HHSequenceSlider::fireWithArg(uint32_t newValue) {
    ...
    for(std::vector<HHCommandBase*>::iterator itr = mCommands.begin(); itr != mCommands.end(); itr++) {
        if ((*itr)->needsArg()) (*itr)->setValue(newValue);
        (*itr)->execute();
    }
...
uint32_t HHSequenceSlider::getMax() { return mMax; }

uint32_t HHSequenceSlider::getMin() { return mMin; }

uint32_t HHSequenceSlider::getStep() { return mStep; }

uint32_t HHSequenceSlider::getValue() { return mValue; }

void HHSequenceSlider::setValue(uint32_t value) { mValue = value; }
