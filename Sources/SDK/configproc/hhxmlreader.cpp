...
bool HHXmlReader::importCommands(std::vector<HHCommandBase *> &commandList, TiXmlDocument *pDom) { 
    ...
    for (TiXmlElement *pEle = pRootEle->FirstChildElement(); NULL != pEle; pEle = pEle->NextSiblingElement()) {
        ...
        for (TiXmlElement* pChildEle = pEle->FirstChildElement(); NULL != pChildEle; pChildEle = pChildEle->NextSiblingElement()) {
    ...
    for (TiXmlElement *pEle = pRootEle->FirstChildElement(); NULL != pEle; pEle = pEle->NextSiblingElement()) {
        ...
        for (TiXmlElement* pChildEle = pEle->FirstChildElement();  NULL != pChildEle; pChildEle = pChildEle->NextSiblingElement()) {
            ...
            if ("command" == commandName || "system" == commandName) {
                ...
                if ((string::npos != cmdName.find("#value#")) && pCmd) pCmd->needsArg(true);
