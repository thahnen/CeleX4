		...
		static int calculateDenoiseScore(const cv::Mat& currImg, int row, int col) {
			...
			for (int i=row-1; i<row+2; ++i) {
				for (int j=col-1; j<col+2; ++j) {
					...
					if (currImg.ptr<uchar>(index / PIXELS_PER_COL)[index % PIXELS_PER_COL] > 0) ++count1; // why ++[...] ?
					else                                                                        ++count2; // why ++[...] ?
				}
			}
			...
			for (int i = 0; i < 4; ++i) {
				...
				if (m_matPreEventPicBuffer.ptr<uchar>(aa[i] / PIXELS_PER_COL)[aa[i] % PIXELS_PER_COL])  ++count3; // why ++[...] ?
				else                                                                                    ++count4; // why ++[...] ?
			}
...
*  @return   :	error code : 1 -> true / 0 -> false
...
void dvs::denoisingByNeighborhood(const cv::Mat& countEventImg, cv::Mat& denoisedImg) {
	...
	for (int i=0; i<countEventImg.rows; ++i) {
		...
		for (int j=0; j<countEventImg.cols; ++j) {
			int score;
			if (p[j] > 0)   score = EventProcessing::calculateDenoiseScore(countEventImg, i, j);
			else            score = 0;
