...
FPGADataProcessor::FPGADataProcessor() : m_bIsGeneratingFPN(false), m_bAdjustBrightness(false), m_iFpnCalculationTimes(0)
										, m_uiFullPicPixelCount(0), m_uiOpticalFlowPixelCount(0), m_uiEventPixelCount(0)
										, m_uiUpperADC(511), m_uiLowerADC(0), m_uiTimeSlice(750000)
										, m_uiTimeSliceValue(60), m_uiClockRate(25), m_uiFPGATimeCycle(131072)
										, m_uiFrameStartT(0), m_uiFrameEndT(0), m_uiFEEventTCounter(0)
										, m_uiFEFrameTime(0), m_uiOverlapTime(0), m_uiMeanIntensity(0)
										, m_ulOverlapEventTCounter(0), m_ulSpecialEventCount(0), m_ulSpecialByteSize(0)
										, m_emSensorMode(EventMode), m_pEventPicBuffer(NULL), m_pLastEventPicBuffer(NULL)
										, m_bMultiSliceEnabled(false), m_bNeedResetMultiSliceData(true), m_uiMultiSliceTime(1250000)
										, m_uiMultiSliceCount(255), m_ulEventTCounter(0), m_ulFixedEventTCounter(0), m_ulMultiSliceTCounter(0)
										, m_ulEventCountTotal(0), m_uiTimeStamp(750000), m_uiTimeScale(1), m_emPlaybackState(CeleX4::NoBinPlaying)
	...
	for (i = 0; i + EVENT_SIZE <= length; i = i + EVENT_SIZE) {
		...
		if (isSpecialEvent) {
			if (m_bAdjustBrightness) {
				...
				if(!isGetFixedVec) m_pCX4Server->notify(CeleX4DataManager::CeleX_Frame_Data);
			}

			if (!isReadBin) cleanEventBuffer();
...
bool FPGADataProcessor::processRowEvent(unsigned char* data) {
	...
	if (m_emSensorMode == EventMode) {
		...
		if (m_uiFEEventTCounter == 0 && diffT > 10000) diffT = 0;

		m_uiFEEventTCounter += diffT;
		m_ulTimeStampCount += diffT;

		if (m_bMultiSliceEnabled) {
			if (0 == FPGADataReader::getEventType()) {
...
void FPGADataProcessor::processColEvent(unsigned char* data) {
	...
	if (row < PIXELS_PER_ROW && col < PIXELS_PER_COL) {
		...
		if (m_emSensorMode == FullPictureMode) {
			...
			if (isGetFixedVec) eData.t = m_ulFixedEventTCounter;
			else eData.t = m_ulTimeStampCount / m_uiTimeScale;
			...
			if(isGetFixedVec) m_fixedNumVecEventData.push_back(eData);
		} else if (m_emSensorMode == FullPic_Event_Mode) {
...
void FPGADataProcessor::createImage() {
	if (FullPictureMode == m_emSensorMode) {
		...
		for (int i = 0; i < PIXELS_NUMBER; ++i) {
			...
			if (m_uiOverlapTime > 0) {
				if (m_pOverlapEventBuffer[i] > 0) {
					...
					if (m_pOverlapEventBuffer[i] < 0) m_pOverlapEventBuffer[i] = 0;
				} else {
					m_pOverlapEventBuffer[i] = 0;
				}
		...
		if (NULL == m_pLastEventPicBuffer) m_pLastEventPicBuffer = new unsigned char[PIXELS_NUMBER];
		...
		if (m_bMultiSliceEnabled) {
			...
			for (int i = 0; i < PIXELS_NUMBER; ++i) {
				if (m_pMultiSliceGrayBuffer[i] > 0) {
					if (0 == calculateDenoiseScore(m_pMultiSliceGrayBuffer, i)) m_pMultiSliceGrayBuffer[i] = 0;
				}
			}
		...
		for (int i = 0; i < PIXELS_NUMBER; ++i) {
			...
			if (score > 0) score = calculateDenoiseScore(m_pBufferMotion, m_pLastEventPicBuffer, i);
			else score = 0;
...
emSensorMode FPGADataProcessor::getSensorMode() { return m_emSensorMode; }
...
void FPGADataProcessor::adjustBrightness() { m_bAdjustBrightness = (!m_bAdjustBrightness); }

void FPGADataProcessor::setUpperADC(uint32_t value) { m_uiUpperADC = value; }

uint32_t FPGADataProcessor::getUpperADC() { return m_uiUpperADC; }

void FPGADataProcessor::setLowerADC(uint32_t value) { m_uiLowerADC = value; }

uint32_t FPGADataProcessor::getLowerADC() { return m_uiLowerADC; }
...
void FPGADataProcessor::setOverlapTime(uint32_t msec) {
	m_uiOverlapTime = m_uiClockRate * 1000 * msec / 2;
}

void FPGADataProcessor::setFrameLengthRange(float startRatio, float endRatio) {
	uint32_t emsec = 0;

	if (m_uiFEFrameTime > 20) emsec = m_uiFEFrameTime - 20;

	m_uiFrameStartT = startRatio * m_uiClockRate * 1000 * emsec / 2;
	m_uiFrameEndT = endRatio * m_uiClockRate * 1000 * emsec / 2;
}

void FPGADataProcessor::setFEFrameTime(uint32_t msec) { m_uiFEFrameTime = msec; }

CeleX4ProcessedData *FPGADataProcessor::getSensorDataObject() { return m_pCX4ProcessedData; }

CX4SensorDataServer *FPGADataProcessor::getSensorDataServer() { return m_pCX4Server; }

void FPGADataProcessor::setClockRate(uint32_t value) {
	m_uiClockRate = value;
	m_uiTimeStamp = m_uiTimeSlice = m_uiClockRate * 1000 * m_uiTimeSliceValue / 2;
}

void FPGADataProcessor::setFPGATimeCycle(uint32_t value) { m_uiFPGATimeCycle = value; }
...
bool FPGADataProcessor::isMultiSliceEnabled() { return m_bMultiSliceEnabled; }

void FPGADataProcessor::setMultiSliceTime(uint32_t msec) {
	m_uiMultiSliceTime = m_uiClockRate * 1000 * msec / 2;
	m_ulMultiSliceTCounter = 0;
	m_bNeedResetMultiSliceData = true;
}

void FPGADataProcessor::setMultiSliceCount(uint32_t count) { m_uiMultiSliceCount = count; }
...
unsigned int FPGADataProcessor::normalizeADC(unsigned int adc) {
	...
	if (adc < m_uiLowerADC) brightness = 0;
	else if (adc > m_uiUpperADC) brightness = 255;
	else brightness = 255 * (adc - m_uiLowerADC) / (m_uiUpperADC - m_uiLowerADC);
...
void FPGADataProcessor::calDirectionAndSpeed() {
	...
	for (int i = 0; i < PIXELS_NUMBER; ++i) {
		...
		if (col == 0 || col == 767) Gx = 0;
		else Gx = m_pMultiSliceGrayBuffer[i + 1] - m_pMultiSliceGrayBuffer[i - 1];

		if (row == 0 || row == 639) Gy = 0;
		else Gy = m_pMultiSliceGrayBuffer[i + 768] - m_pMultiSliceGrayBuffer[i - 768];
...
int FPGADataProcessor::calculateDenoiseScore(unsigned char* pBuffer, unsigned char* pBufferPre, unsigned int pos) {
	if (NULL == pBuffer || NULL == pBufferPre) return 255;
	...
	for (int i = row - 1; i < row + 2; ++i) {
		for (int j = col - 1; j < col + 2; ++j) {
			...
			if (index < 0 || index == pos || index >= PIXELS_NUMBER) continue;
			
			if (pBuffer[index] > 0) ++count1;
			else ++count2;
		}
	}
...
unsigned long FPGADataProcessor::getSpecialEventCount() { return m_ulSpecialEventCount; }

void FPGADataProcessor::setSpecialEventCount(unsigned long count) { m_ulSpecialEventCount = count; }

std::vector<int> FPGADataProcessor::getDataLengthPerSpecial() { return m_vecDataLength; }

std::vector<unsigned long> FPGADataProcessor::getEventCountListPerSpecial() { return m_vecEventCountPerSpecial; }

uint32_t FPGADataProcessor::getMeanIntensity() { return m_uiMeanIntensity; }
...
BinFileAttributes FPGADataProcessor::getBinFileAttributes(std::string binFile) {
	...
	//open the bin file
	if (m_ifstreamBin.is_open()) m_ifstreamBin.close();
...
void FPGADataProcessor::setTimeScale(float scale) { m_uiTimeScale = scale * 500 * m_uiClockRate; }
...
Return:	void
*************************************************/
void FPGADataProcessor::setEventCountStepSize(uint32_t stepSize) { m_uiEventCountStepSize = stepSize; }

int FPGADataProcessor::getIMUDataSize() { return m_vectorIMUData.size(); }
...
bool FPGADataProcessor::denoisingByTimeInterval(std::vector<EventData> vec, cv::Mat &mat) {
	...
	if (!vec.empty()) {
		...
		for (int t = 0; t < vec.size() - 1; ++t) {
			if (vec[t].row == 0 || vec[t].row == 639 || vec[t].col == 0 || vec[t].col == 767) continue;
			...
			if (deltaT < m_fTSum / m_lCount / 2.0) mat.at<uchar>(640 - vec[t].row - 1, 768 - vec[t].col - 1) = 255;
