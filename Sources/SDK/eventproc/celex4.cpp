...
// TODO: Maybe delete some of the useless cout's
// TODO: combine functions with standard values!
...
long CeleX4::readDataFromFPGA(long length, unsigned char *data) {
	if (!data || !isSensorReady()) return -1;
...
void CeleX4::enableADC(bool enable) {
	if (enable) excuteCommand("ADC Disable");
	else        excuteCommand("ADC Enalbe");
}
...
void CeleX4::stopRecordingVideo() {
	...
	if (g_cvVideoWriterFullPic.isOpened())  g_cvVideoWriterFullPic.release();
	if (g_cvVideoWriterEvent.isOpened())    g_cvVideoWriterEvent.release();
...
bool CeleX4::readPlayBackData(long length) {
	...
	if (NULL == m_pReadBuffer) m_pReadBuffer = new unsigned char[maxLen];
	...
	while (true && m_pDataProcessThread->queueSize() < 1000000) {
		...
		if (dataLen > 0) m_pDataProcessThread->addData(m_pReadBuffer, dataLen);
...
bool CeleX4::openPlaybackFile(string filePath) {
	...
	if (header[7] >= 0) m_pDataProcessThread->setSensorMode((emSensorMode)header[7]);
	else                m_pDataProcessThread->setSensorMode(EventMode);
...
void CeleX4::pause() {
	if (m_pDataProcessThread->isRunning()) m_pDataProcessThread->suspend();
}
...
void CeleX4::parseSliderSequence() {
	...
	for (vector<string>::iterator itr = sliderNames.begin(); itr != sliderNames.end(); itr++) {
		...
		if (!pSliderSeq || !pSliderSeq->isShown()) continue;
...
bool CeleX4::excuteCommand(string strCommand) {
	...
	if (HHSequence* pUpSeq = m_pSequenceMgr->getSequenceByName(strCommand)) {
		if (!pUpSeq->fire())    cout << "excute command failed." << endl;
		else                    bSuccess = true;
	} else {
...
void CeleX4::autoAdjustBrightness() {
	...
	if (meanIntensity < 110 || meanIntensity > 140) {
		...
		if (meanIntensity > 220)        brightness -= 10;
		else if (meanIntensity > 200)   brightness -= 5;
		else if (meanIntensity > 180)   brightness -= 2;
		else if (meanIntensity > 150)   brightness -= 1;
		else if (meanIntensity > 140)   brightness -= 1;
		else if (meanIntensity < 50)    brightness += 10;
		else if (meanIntensity < 80)    brightness += 5;
		else if (meanIntensity < 110)   brightness += 1;

		if (meanIntensity > 1000 || meanIntensity < 10) brightness = 250;
...
bool CeleX4::getFixedNumEventDataVec(std::vector<EventData>& vector, uint64_t& frameNo) {
...
cv::Mat CeleX4::getFullPicMat() {
	CeleX4ProcessedData* pSensorData = getSensorDataObject();
	if (!pSensorData) return cv::Mat();

	return cv::Mat(cv::Size(768, 640), CV_8UC1, pSensorData->getFullPicBuffer());
}
...
cv::Mat CeleX4::getEventPicMat(emEventPicMode mode) {
	...
	return cv::Mat(cv::Size(768, 640), CV_8UC1, pSensorData->getEventPicBuffer(mode));
}
...
cv::Mat CeleX4::getOpticalFlowPicMat() {
	if (isOpticalFlowEnabled()) {
		...
		return cv::Mat(cv::Size(768, 640), CV_8UC1, pSensorData->getOpticalFlowPicBuffer());
	}
	return cv::Mat();
}
...
cv::Mat CeleX4::getOpticalFlowDirectionPicMat() {
	if (isOpticalFlowEnabled()) {
		...
		return cv::Mat(cv::Size(768, 640), CV_8UC1, pSensorData->getOpticalFlowDirectionPicBuffer());
	}
	return cv::Mat();
}
...
cv::Mat CeleX4::getOpticalFlowSpeedPicMat() {
	if (isOpticalFlowEnabled()) {
		...
		return cv::Mat(cv::Size(768, 640), CV_8UC1, pSensorData->getOpticalFlowSpeedPicBuffer());
	}
	return cv::Mat();
}
...
void CeleX4::convertBinToAVI(std::string binFile, emEventPicMode picMode, uint32_t frameTime, uint32_t intervalTime, cv::VideoWriter writer) {
	...
	if (sensorMode == EventMode) m_pDataProcessThread->getDataProcessor()->setOverlapTime(frameTime - intervalTime);	//set overlaptime for event
